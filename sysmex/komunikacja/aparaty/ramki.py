...
class Order:
    ...
    def uzupelnij_dane(self, aparat, ob, testy_do_aparatu):
        ...
        # jeżeli jest pusty pesel to daję datę urodzenia + ID_PACJENTA
        if '      ' in z_info['PESEL']:
            self.parent.pesel = '{data}{id_pac}'.format(
                            data=self.parent.data_urodzenia,
                            id_pacj=z_info['ID_PACJENTA']
                            )[:11]
        else:
            self.parent.pesel = z_info['PESEL']
